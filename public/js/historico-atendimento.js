...
async function carregarHistoricoAtendimento() {
    try {
        const response = await fetch('/api/historico/atendimento');
...
function exibirHistoricoAtendimento() {
    ...
    recordsToShow.forEach(registro => {
        ...
        row.innerHTML = `
            ...
            <td>#${registro.IdAtendimento || 'N/A'}</td>
            <td>${registro.NomeResponsavel || 'N/A'}</td>
            <td>${registro.NomeCuidador || 'N/A'}</td>
            <td>${registro.NomeIdoso || 'N/A'}</td>
            <td>
                <span class="status-badge status-${getClasseStatus(registro.Operacao)}">
                    ${registro.Operacao}
                </span>
            </td>
            <td>${dataFormatada}</td>
...
function getClasseStatus(status) {
    ...
    if (statusLower.includes('agendado') || statusLower.includes('pendente')) {
        return 'pendente';
    } else if (statusLower.includes('em andamento') || statusLower.includes('ativo')) {
        return 'ativo';
...
function buscarHistorico(termo) {
    if (!termo.trim()) {
        ...
        allRecords = allRecords.filter(registro => 
            ...
            (registro.NomeCuidador && registro.NomeCuidador.toLowerCase().includes(termo.toLowerCase())) ||
            (registro.NomeIdoso && registro.NomeIdoso.toLowerCase().includes(termo.toLowerCase()))
        );
    }

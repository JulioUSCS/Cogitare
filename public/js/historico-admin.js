...
// Função para carregar histórico de administradores
async function carregarHistoricoAdmin() {
    try {
        const response = await fetch('/api/historico/administrador');
...
function buscarHistorico(termo) {
    if (!termo.trim()) {
        ...
        allRecords = allRecords.filter(registro => 
            ...
            (registro.NomeAdministrador && registro.NomeAdministrador.toLowerCase().includes(termo.toLowerCase()))
        );
    }
